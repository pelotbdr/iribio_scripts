...
a=rownames(all_genes[all_genes$DE_status!=0,])
fileConn=file("/media/beaudier/OPTIC/NANOS_ARTICLE/starved_degenes.txt")
writeLines(a,fileConn)
close(fileConn)

b=all_genes[all_genes$DE_status!=0,]
write.csv(b,'300gy_DE_table.csv')


fileConn=file("300gy_genes.txt")
writeLines(a3,fileConn)
close(fileConn)



b=gprofil(a,'celegans')$result
b=b[b$source=='GO:BP',]$term_name
b <- apply(b,2,as.character)
write.csv(b,'/media/beaudier/OPTIC/sarcome_3runs/ib106/irr2G_gprofil.csv')
